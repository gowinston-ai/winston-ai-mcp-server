...
describe("Winston AI Client", () => {
  ...
  const GENERATED_SAMPLE_TEXT =
    "Whales are some of the largest and most fascinating creatures on Earth. Belonging to the order Cetacea, these majestic marine mammals inhabit oceans all around the world.";
  ...
  const AI_CONTENT_DETECTION_EXPECTED_ERROR_MESSAGE =
    "There was an error while detecting the AI content in the text. Make sure your text is longuer than 300 characters and it is in a supported language. We currently support English, French, Spanish, German, Italian, Portuguese, Dutch, Tagalog, Italian, Polish and Indonesian.";
  const AI_IMAGE_DETECTION_EXPECTED_ERROR_MESSAGE =
    "There was an error while detecting the AI content in the image. Make sure the URL is valid and the image is at least 256x256 pixels.";
  const PLAGIARISM_DETECTION_EXPECTED_ERROR_MESSAGE =
    "There was an error while detecting the plagiarism in the text, please try again.";
  const TEXT_COMPARE_EXPECTED_ERROR_MESSAGE =
    "There was an error while detecting the similarity between the two texts, please try again.";
  ...
  describe("assembleAiTextDetectorResponse", () => {
    describe("given a result with a error", () => {
      ...
      it("should return an error message", () => {
        const result =
          winstonAIClient.assembleAiTextDetectorResponse(errorResult);
    ...
    describe("given a valid response", () => {
      ...
      beforeEach(() => {
        result = winstonAIClient.assembleAiTextDetectorResponse(
          AI_TEXT_DETECTION_VALID_RESPONE,
        );
      });

      it("then result sould contain the explanation text", () => {
        expect(result).toContain(
          "The AI detector Winston AI has detected the text as 65% human-written. Which means that the text is 35% likely to be written by an AI.",
        );
      });
  ...
  describe("assembleAiImageDetectorResponse", () => {
    describe("given a result with a error", () => {
      const errorResult = {
        error:
          "There was an error while detecting the AI content in the image.",
      };

      it("should return an error message", () => {
        const result =
          winstonAIClient.assembleAiImageDetectorResponse(errorResult);
    ...
    describe("given a valid response", () => {
      ...
      beforeEach(() => {
        result = winstonAIClient.assembleAiImageDetectorResponse(
          AI_IMAGE_DETECTION_VALID_RESPONSE,
        );
      });

      it("then result should contain the explanation text", () => {
        expect(result).toContain(
          "The AI detector Winston AI has detected the image as 99.94% human. Which means that the image is 0.06% likely to be AI generated.",
        );
      });
  ...
  describe("assemblePlagiarismResponse", () => {
    ...
    describe("given a valid response", () => {
      ...
      beforeEach(() => {
        result = winstonAIClient.assemblePlagiarismResponse(
          PLAGIARISM_DETECTION_VALID_RESPONSE as unknown as PlagiarismDetectionResponse,
        );
      });

      it("then result should contain the explanation text", () => {
        expect(result).toContain(
          'The plagiarism detection tool Winston AI has detected the text as 71% plagiarism. The main plagiarized sources are """URL""" with a plagiarism score of 71% plagiarism',
        );
      });

      it("then result should contain the sources", () => {
        expect(result).toContain(
          '"""URL""" with a plagiarism score of 71% plagiarism',
        );
      });
  ...
  describe("assembleTextCompareResponse", () => {
    describe("given a result with a error", () => {
      const errorResult = {
        error:
          "There was an error while detecting the similarity between the two texts.",
      };
    ...
    describe("given a valid response", () => {
      ...
      beforeEach(() => {
        result = winstonAIClient.assembleTextCompareResponse(
          TEXT_COMPARE_VALID_RESPONSE,
        );
      });

      it("then result should contain the explanation text", () => {
        expect(result).toContain(
          "The similarity score between the two texts is 100%. The first text is 100% similar to the second text. The second text is 100% similar to the first text.",
        );
      });
